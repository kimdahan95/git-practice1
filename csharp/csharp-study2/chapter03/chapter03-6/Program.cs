...
class Program
{
    static void Main(string[] args)
    {
        ...
        // 배열(array)
        int[] productPrices = new int[4];  // 필요한 만큼만 할당하는게 best
        ...
        //productPrices;  // 빌드(build) 에러

        int i = 0;
        productPrices[i] = 5200;

        Console.WriteLine(productPrices[0]);  // 5200 ,  [0] <- 색인(index)
        ...
        //문자열에 대한 배열
        string text = "Hello World";  // 총 11개의 배열로 이루어짐
        char ch1 = text[0];  // H
        char ch2 = text[1];  // e
        char ch11 = text[10];// d
        //char ch12 = text[11]; //error -> text 변수 문자열 11개까지 (0~10)만 저장이 되어 있음
        ...
        string[]abc = new string[60];
        Console.WriteLine(abc[0]);       // 출력 결과 : null
        ...
        Console.WriteLine(arr4[2,1]);  // 출력 결과 : 8

        // 가변배열(배열의 배열)
        int[][]arr5= new int[5][];    //5개의 열을 가진 가변 배열
        arr5[0] = new int[10];        //0번째의 index의 10개의 열을 가진 배열
        arr5[1] = new int[9];
        arr5[2] = new int[8];
        arr5[3] = new int[3];
        arr5[4] = new int[5];

        //문제1 : 아래 코드 arr6의 평균값을출력하는 코드를 작성하시오.(소수점까지 표시)
        ...
        Console.WriteLine((c1+c2+c3+c4+c5+c6)/6);
